...
Windows 关闭窗口会隐藏到托盘，可从托盘恢复或退出；macOS 保持官方窗口行为。官方自动更新、强制更新、认证页和其他 Desktop 功能保持启用。
...
`dist/electron/` 生成 DMG 或 NSIS EXE，以及包含官方 commit、实际版本和插件版本的 `build-info-official.json`。`npm run prepare:runtime` 调用官方 `prepare:package --prepare-only`；`npm run build` 继续调用官方准备流程、官方 Desktop build、官方 electron-builder 配置和官方 packaged-runtime smoke，本地社区包使用官方 unsigned 分支，CI 配置签名凭据后可走官方 signed 分支。`npm run test:app` 通过 `OWNDSH_TEST_APP` 指定实际打包程序，用临时独立目录验证完整性、启动、原生模块、Word 转 PDF、插件 Server 保存、未登录卸载和重启。
