...
当前锁定 Pake 3.16.1、Harness 0.1.7-rc.1、OwnDsh 插件 0.1.0-beta.11、Node 24.14.1 与 pnpm 11.26.0。
...
启动时如果发现 `.credentials.yaml.lock` 记录的 PID 已退出，桌面启动器会清理该陈旧锁并继续启动；PID 仍存活或锁内容无法确认时会保留锁，避免并发实例互相破坏凭据。
